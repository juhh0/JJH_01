...
def sortGapInsertion(A, first, last, gap):
    """
    gap 간격을 기준으로 삽입 정렬을 수행하며 비교 및 이동 횟수를 계산
    """
    comparisons = 0  # 비교 횟수
    movements = 0    # 이동 횟수
    ...
    for i in range(first + gap, last + 1, gap):
        ...
        while j >= first and key < A[j]:
            comparisons += 1  # 비교 횟수 증가
            A[j + gap] = A[j]  # 데이터를 오른쪽으로 이동
            movements += 1  # 이동 횟수 증가
            j = j - gap
        if j >= first:
            comparisons += 1  # 마지막 실패한 비교
        A[j + gap] = key  # 삽입 위치에 데이터 저장
        movements += 1  # 삽입 이동
...
def shell_sort(A):
    """
    셸 정렬 구현 (gap이 짝수일 경우 1을 더하는 방식)
    """
    n = len(A)
    gap = n // 2  # 초기 간격 설정
    total_comparisons = 0  # 전체 비교 횟수
    total_movements = 0    # 전체 이동 횟수

    while gap > 0:
        if gap % 2 == 0:  # gap이 짝수이면 1을 더해 홀수로 변경
            gap += 1

        gap_comparisons = 0  # 현재 간격에서의 비교 횟수
        gap_movements = 0    # 현재 간격에서의 이동 횟수

        # 현재 간격에 대해 각 서브리스트 삽입 정렬 수행
        for i in range(gap):
            comparisons, movements = sortGapInsertion(A, i, n - 1, gap)
            gap_comparisons += comparisons
            gap_movements += movements

        # 간격 단계별 비교 및 이동 횟수 출력
        total_comparisons += gap_comparisons
        total_movements += gap_movements

        # 간격 감소
        gap = gap // 2

    return A, total_comparisons, total_movements
